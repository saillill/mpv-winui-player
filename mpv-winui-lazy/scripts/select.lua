...
mp.add_key_binding(nil, "select-track", function ()
    ...
    for i, track in ipairs(mp.get_property_native("track-list")) do
        tracks[i] = (track.image and "图像" or
                     (track.type == "video" and "视频" or
                      track.type == "audio" and "音轨" or "字幕")) .. ": " ..
                    format_track(track)
    end
...
mp.add_key_binding(nil, "select-sid", function ()
    select_track("sid", "sub", "选择字幕：", "没有可用字幕。")
end)

mp.add_key_binding(nil, "select-secondary-sid", function ()
    select_track("secondary-sid", "sub", "选择次字幕：",
                 "没有可用字幕。")
end)
...
mp.add_key_binding(nil, "select-watch-history", function ()
    ...
    if not history_file then
        show_warning(mp.get_property_native("save-watch-history")
                     and error_message
                     or "启用 --save-watch-history 后可跳转最近播放的文件。")
        return
    end
...
mp.add_key_binding(nil, "select-watch-later", function ()
    ...
    if #files == 0 then
        show_warning(mp.get_property_native("write-filename-in-watch-later-config")
            and "没有找到稍后观看文件。"
            or "启用 --write-filename-in-watch-later-config 后可选择最近文件。")
        return
    end
...
local function edit_config_file(filename)
    if not mp.get_property_bool("config") then
        show_warning("--no-config 模式下不支持编辑配置文件。")
        return
    end
...
mp.add_key_binding(nil, "menu", function ()
    ...
    local menu = {
        ...
        {"次字幕", "script-binding select/select-secondary-sid", sub_track_count > 1},
        {"字幕行", "script-binding select/select-subtitle-line", text_sub_selected},
        {"音轨", "script-message-to dyn_menu audio-menu", audio_track_count > 1},
        {"视频轨", "script-binding select/select-vid", video_track_count > 1},
        {"播放列表", "script-message-to dyn_menu playlist-menu",
        ...
        {"稍后观看", "script-binding select/select-watch-later", true},

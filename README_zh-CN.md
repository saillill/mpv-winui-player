...
- **设置窗口**：左侧分类、右侧内容双栏排版（常规/播放/视频/音频/字幕/截屏/路径），共约 35 项 PotPlayer 式选项。所有选项都是可选项且显示本地化标签（不再显示 `auto`/`yes` 等原始键值），音频设备为从 mpv 实时枚举的下拉列表，截图文件名模板提供预设；开关显示本地化的是/否；语言下拉按各语言母语名称显示（中文 / 日本語 / 한국어…）；路径类选项带由设置窗口自己拥有的系统文件夹选择“浏览”按钮；每项说明以 Windows 设置风格显示在标题下方。改动即时下发 mpv，需要重启的设置（如语言）弹出系统提示。
...
| 播放器设置 | 选项较少 | 约 35 项分类选项，即时下发并在启动时应用：硬解、音量/最大音量、keep-open、循环文件/播放列表、速度、反交错、画面比例、放大/缩小算法、旋转、去色带、视频同步、插帧、HR seek、色调映射、抖动、音频语言/设备（实时下拉）/声道/延迟/独占/音调/降混、字幕字号/位置/延迟/语言/ASS 覆盖/模糊/字体/编码、截图目录/模板（预设）/格式/JPEG/PNG/WebP/色深、缓存目录；选项值全部本地化，路径带文件夹选择，说明显示在标题下方 |

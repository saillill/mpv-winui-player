...
- 多种界面语言：中（简/繁）、英、日、韩、德、法、西、俄，随时切换立即生效。
...
打开方式随你挑：菜单、拖拽、命令行参数，或者 `mpv-winui://` 链接（安装版已注册
协议）。凡是 mpv 能播的它都能播——本地文件、网络串流、DVD、蓝光都行。有硬件
解码就默认开着，HDR/广色域内容自动适配，不用手动调颜色。

播放、暂停、进度、音量、倍速、上下首、循环、随机，音轨和字幕随意切换，还能
加载外挂字幕、调整字幕样子。播放列表侧栏支持拖拽排序；进度条上能看到缩略图
预览；记得住上次看到哪，有观看历史和"稍后观看"。截图、文件信息（MediaInfo）、
快捷键搜索，也都齐了。

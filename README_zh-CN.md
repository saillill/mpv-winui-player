...
- **设置窗口**：左侧分类、右侧内容双栏排版（常规/播放/视频/音频/字幕/截屏/高级，路径类选项并入高级），共约 84 项 PotPlayer 式选项。列表类选项显示本地化预设（不再只显示原始键值），并附一个“自定义”选项，选中后才出现录入框；音频设备为从 mpv 实时枚举的列表，截图文件名模板提供预设；开关显示本地化的开/关；语言下拉按各语言母语名称显示（中文 / 日本語 / 한국어…）；路径类选项（截图目录、缓存目录、稍后观看数据目录、ICC 缓存、着色器缓存）带由设置窗口自己拥有的系统文件夹选择“浏览”按钮，并有“打开”按钮直接跳转文件管理器；各分类内按主题归类（如高级里 ICC 选项在一起、字幕选项在一起、目录选项在一起），不再显示插件分组；说明以 Windows 设置风格显示在标题下方，仅在能补充信息时显示、不与黄色警告重复，并给出推荐值与已知冲突。会冲突的选项直接置灰不可选，可能无效的选项显示黄色提示（如帧插值需配合特定同步模式、截图质量项与当前格式不符）。改动即时下发 mpv，语言、背景、调试日志与视频预览等应用级设置也即时生效，无需重启。
...
| 播放器设置 | 选项较少 | 约 84 项分类选项，即时下发并在启动时应用：硬解、音量/最大音量、keep-open、循环文件/播放列表、速度、保存/恢复播放进度、反交错、画面比例、放大/缩小算法、旋转、去色带、视频同步、帧插值、HR seek（含丢帧）、色调映射、抖动、音频语言/设备（实时列表）/声道/延迟/独占/音调/降混/音频文件显示、字幕字号/位置/延迟/语言/ASS 覆盖/模糊/字体（系统默认 + 微软字体）/字体提供程序/编码/回退/混合/随窗口缩放/ASS 黑边/图形字幕拉伸、OSD 字体/字号/跳转显示/时长、ICC 自动校色与 3D LUT、视频输出电平、解码直接输出到显存（`vd-lavc-dr`）、解复用器缓存（`demuxer-max-bytes`）、磁盘缓存、截图目录/模板（预设）/格式/JPEG/PNG/WebP/色深/软件截图、缓存目录、稍后观看数据目录、ICC 缓存目录、着色器缓存目录、NVIDIA VSR / RTX Video HDR / 拖动保持窗口开关；选项值全部本地化，预设 + 自定义选项，冲突项禁用，可能无效项黄色提示，路径带文件夹选择与打开按钮，说明去重且含推荐 |

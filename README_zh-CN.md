...
- **设置窗口**：左侧分类、右侧内容双栏排版（常规/播放/视频/音频/字幕/截屏/高级/路径），共约 78 项 PotPlayer 式选项。所有选项都是可选项且显示本地化标签（不再显示 `auto`/`yes` 等原始键值），音频设备为从 mpv 实时枚举的下拉列表，截图文件名模板提供预设；开关显示本地化的开/关；语言下拉按各语言母语名称显示（中文 / 日本語 / 한국어…）；路径类选项带由设置窗口自己拥有的系统文件夹选择“浏览”按钮，并有“打开”按钮直接跳转文件管理器；说明以 Windows 设置风格显示在标题下方，仅在能补充信息时显示，并给出推荐值与已知冲突。每项带来源标记（**MPV** / **插件** / **应用** / **混合**），脚本类选项集中在各分类末尾的“插件选项”小节；会冲突的选项直接置灰不可选，可能无效的选项显示黄色提示（如插帧需配合特定同步模式、截图质量项与当前格式不符）。改动即时下发 mpv，需要重启的设置（如语言）弹出系统提示。

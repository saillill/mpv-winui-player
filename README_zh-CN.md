...
- **右键菜单**：mpv 数据菜单（153 项，tsl0922/mpv-menu 排版）+ 固定文件/窗口项；“滤镜与增强”下直接显示 Nvidia VSR / RTX Video HDR / 清空所有脚本；菜单标题与动态项在 8 种语言下全量翻译（`dyn_menu.lua` / `dynamic_menu.lua` 内置翻译表），经 `user-data/mpvw/language` 自动切换。
- **设置窗口**：左侧分类、右侧内容双栏排版（常规/播放/视频/音频/字幕/截屏/路径），共约 30 项 PotPlayer 式选项。所有选项都是可选项且显示本地化标签（不再显示 `auto`/`yes` 等原始键值）；开关显示本地化的是/否；语言下拉按各语言母语名称显示（中文 / 日本語 / 한국어…）；路径类选项带系统文件夹选择“浏览”按钮；生僻选项带 ⓘ 帮助按钮解释用途。改动即时下发 mpv，需要重启的设置（如语言）弹出系统提示。
...
| 播放器设置 | 选项较少 | 约 30 项分类选项，即时下发并在启动时应用：硬解、音量/最大音量、keep-open、循环文件/播放列表、速度、反交错、画面比例、放大/缩小算法、旋转、去色带、视频同步、插帧、HR seek、色调映射、抖动、音频语言/设备/声道/延迟/独占/音调/降混、字幕字号/位置/延迟/语言/ASS 覆盖/模糊/字体/编码、截图目录/模板/格式/JPEG/PNG/WebP/色深、缓存目录；选项值全部本地化，路径带文件夹选择，生僻项带帮助图标 |
...
硬解（`hwdec`）、启动音量/最大音量（`volume`/`volume-max`）、播放结束动作（`keep-open`）、循环文件/播放列表（`loop-file`/`loop-playlist`）、默认速度、反交错、画面比例、放大/缩小算法（`scale`/`dscale`）、旋转、去色带、线性缩小、S 形放大、视频同步、插帧、HR seek、HDR 色调映射、抖动深度、首选音频/字幕语言（`alang`/`slang`）、音频设备、声道、延迟、独占模式、音调校正、降混标准化、自动加载外挂音频、字幕字号/位置/延迟/字体/编码/描边/阴影/ASS 覆盖/模糊/内嵌字体/黑边、截图目录（带系统文件夹选择）与文件名模板/格式/JPEG 质量/PNG 压缩/WebP 质量/高色深/色彩空间标记、缓存目录、视频预览缩略图。所有选项值均显示本地化标签，生僻选项带 ⓘ 帮助说明。改动即时下发 mpv，启动时自动应用；需要重启的设置（如语言）弹出“立即重启”系统提示。

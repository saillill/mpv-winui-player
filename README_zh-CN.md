...
- **设置窗口**：左侧分类、右侧内容双栏排版（常规/播放/视频/音频/字幕/截屏/高级，路径类选项并入高级），共约 84 项 PotPlayer 式选项。列表类选项显示本地化预设（不再只显示原始键值），并附一个“自定义”选项，选中后才出现录入框；音频设备为从 mpv 实时枚举的列表，截图文件名模板提供预设；开关显示本地化的开/关；语言下拉按各语言母语名称显示（中文 / 日本語 / 한국어…）；路径类选项（截图目录、缓存目录、稍后观看数据目录、ICC 缓存、着色器缓存）带由设置窗口自己拥有的系统文件夹选择“浏览”按钮，并有“打开”按钮直接跳转文件管理器；各分类内按主题归类（如高级里 ICC 选项在一起、目录选项在一起），字幕分类把 ASS/高级字幕选项（ASS 覆盖、ASS 黑边、ASS 随窗口缩放、内嵌字体、字幕混合、字幕回退）单独分成一节；不再显示插件分组；说明以 Windows 设置风格显示在标题下方，仅在能补充信息时显示、不与黄色警告重复，并给出推荐值与已知冲突。会冲突的选项直接置灰不可选，可能无效的选项显示黄色提示（如帧插值需配合特定同步模式、截图质量项与当前格式不符）。改动即时下发 mpv，语言、背景、调试日志与视频预览等应用级设置也即时生效，无需重启。

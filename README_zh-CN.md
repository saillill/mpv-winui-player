...
- **设置窗口**：左侧分类、右侧内容双栏排版（常规/播放/视频/音频/字幕/截屏/高级/路径），共约 70 项 PotPlayer 式选项。所有选项都是可选项且显示本地化标签（不再显示 `auto`/`yes` 等原始键值），音频设备为从 mpv 实时枚举的下拉列表，截图文件名模板提供预设；开关显示本地化的开/关；语言下拉按各语言母语名称显示（中文 / 日本語 / 한국어…）；路径类选项带由设置窗口自己拥有的系统文件夹选择“浏览”按钮，并有“打开”按钮直接跳转文件管理器；说明以 Windows 设置风格显示在标题下方，仅在能补充信息时显示。改动即时下发 mpv，需要重启的设置（如语言）弹出系统提示。
...
| 播放器设置 | 选项较少 | 约 70 项分类选项，即时下发并在启动时应用：硬解、音量/最大音量、keep-open、循环文件/播放列表、速度、反交错、画面比例、放大/缩小算法、旋转、去色带、视频同步、插帧、HR seek、色调映射、抖动、音频语言/设备（实时下拉）/声道/延迟/独占/音调/降混/音频文件显示、字幕字号/位置/延迟/语言/ASS 覆盖/模糊/字体（系统默认 + 微软字体）/编码/回退/混合/随窗口缩放、OSD 字号/时长、ICC 自动校色与 3D LUT、视频输出电平、磁盘缓存、截图目录/模板（预设）/格式/JPEG/PNG/WebP/色深、缓存目录、NVIDIA VSR / RTX Video HDR / 拖动保持窗口开关；选项值全部本地化，路径带文件夹选择与打开按钮，说明去重显示 |
...
硬解（`hwdec`）、启动音量/最大音量（`volume`/`volume-max`）、播放结束动作（`keep-open`）、循环文件/播放列表（`loop-file`/`loop-playlist`）、默认速度、反交错、画面比例、放大/缩小算法（`scale`/`dscale`）、旋转、去色带、线性缩小、S 形放大、视频同步、插帧、HR seek、HDR 色调映射、抖动深度、首选音频/字幕语言（`alang`/`slang`）、音频设备、声道、延迟、独占模式、音调校正、降混标准化、自动加载外挂音频、音频文件显示（`audio-display`）、字幕字号/位置/延迟/字体（系统默认、Segoe UI、微软雅黑、Arial、Times New Roman、Consolas、随包思源黑体/LXGW WenKai）/编码/描边/阴影/ASS 覆盖/模糊/内嵌字体/黑边/回退（`subs-fallback`）/混合模式（`blend-subtitles`）/随窗口缩放、OSD 字号与显示时长、ICC 自动校色与 3D LUT 大小、视频输出电平、磁盘缓存、截图目录（系统文件夹选择 + 打开资源管理器）与文件名模板/格式/JPEG 质量/PNG 压缩/WebP 质量/高色深/色彩空间标记、缓存目录、视频预览缩略图。新增**高级**分类收纳底层色彩/流/脚本选项：磁盘缓存、视频输出电平、ICC 配置、ICC 3D LUT、音频文件显示、字幕回退、字幕混合、ASS 随窗口缩放、OSD 字号/时长、自动 NVIDIA VSR、RTX Video HDR 模式、拖动进度保持窗口大小。所有选项值均显示本地化标签，说明仅在补充信息时显示在标题下方。改动即时下发 mpv，启动时自动应用；需要重启的设置（如语言）弹出“立即重启”系统提示。

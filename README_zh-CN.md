...
| 播放器设置 | 选项较少 | 约 114 项分类选项，即时下发并在启动时应用：硬解、音量/最大音量、keep-open、循环文件/播放列表、速度、保存/恢复播放进度、反交错、画面比例、放大/缩小/色度上采样/帧插值算法、旋转、去色带、视频同步、帧插值、HR seek（含丢帧）、色调映射、抖动、panscan、网络缓存与预读、yt-dlp 解析、自动播放列表/文件夹模式、音频语言/设备（实时列表）/声道/延迟/独占/音调/降混/音频文件显示/封面、字幕字号/位置/延迟/语言/外挂目录/HDR 峰值/ASS 覆盖与样式覆盖/模糊/字体（系统默认 + 微软字体）/字体提供程序/编码/回退/混合/随窗口缩放/ASS 黑边/图形字幕拉伸、OSD 字体/字号/跳转显示/时长/播放消息/进度条宽度、目标色彩空间/基色/传递曲线/峰值、ICC 自动校色与 3D LUT 及缓存、视频输出电平、解码直接输出到显存（`vd-lavc-dr`）、解复用器前向/后向缓存、磁盘缓存、截图目录/模板（预设）/格式/JPEG/PNG/WebP/色深/软件截图、缓存目录、稍后观看数据目录、ICC 缓存目录、着色器缓存目录、NVIDIA VSR / RTX Video HDR / 拖动保持窗口开关，以及 script-opts 插件选项（自动播放列表、元数据 OSD、封面、thumbfast、RTX HDR 日志）；选项值全部本地化，预设 + 自定义选项，冲突项禁用，可能无效项黄色提示，路径带文件夹选择与打开按钮，说明去重且含推荐 |

...
| mpv 选项 | 为什么值得加 |
|---|---|
| `osd-back-color` | OSD 文字背后的底色。 |
| `osd-bar` | 是否显示进度条。 |
| `osd-bar-align-x` | 进度条的水平位置。 |
| `osd-bar-align-y` | 进度条的垂直位置。 |
| `osd-bar-marker-style` | 进度条上章节标记的样式。 |
| `osd-bold` | OSD 文字加粗。 |
| `osd-font-provider` | OSD 字形用哪个字体后端。 |
| `osd-fonts-dir` | OSD 的额外字体目录。 |
| `osd-italic` | OSD 文字倾斜。 |
| `osd-justify` | OSD 文字对齐方式。 |
| `osd-margin-y-offset` | OSD 上下留白的偏移量。 |
| `osd-scale` | OSD 整体大小。 |
| `osd-scale-by-window` | OSD 随窗口缩放。 |
| `osd-selected-color` | OSD 里选中项的高亮色。 |
| `osd-selected-outline-color` | 选中项高亮的描边色。 |
| `osd-shadow-offset` | OSD 文字阴影。 |
| `osd-shaper` | OSD 的字形整形。 |
| `osd-spacing` | OSD 字距。 |

...
**触发关键词「全面体检」**（同义：深度巡检 / repo-checkup，亦可输入
`/repo-checkup`）：等价于按本文件 + AGENTS.md 执行一轮完整的
基线核对 → 文档怀疑论验证 → 死代码/门控扫描 → 像素级与 IPC 实证 →
修复 → 部署（锁文件守则）→ 推送 → 本文件迭代。完整流水线与红线
固化在维护者个人技能库 `repo-checkup/SKILL.md`。

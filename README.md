...
- **Settings window**: two-pane layout (left categories, right options) with General / Playback / Video / Audio / Subtitle / Screenshot / Paths. It ships ~35 PotPlayer-style options — every option uses a selectable list with localized labels (never raw mpv values), including a runtime audio-device dropdown populated from mpv and screenshot filename-template presets; boolean switches show localized Yes/No, the language list shows each language in its own name (中文 / 日本語 / 한국어 ...), path options have a native Windows folder-picker "Browse" button owned by the settings window, and each option shows its explanation as a caption under the title (Windows Settings style). Changes apply to mpv live; restart-required settings (e.g. language) show a restart prompt.
...
| Player settings | Minimal | ~35 categorized options applied live and on startup: hwdec, volume/volume-max, keep-open, loop file/playlist, speed, deinterlace, aspect, scaling/downscaling algorithms, rotation, deband, video sync, interpolation, HR seek, tone mapping, dithering, audio language/device (live dropdown)/channels/delay/exclusive mode/pitch/downmix, subtitle size/position/delay/language/ASS override/blur/font/codepage, screenshot directory/template (presets)/format/JPEG/PNG/WebP/bit depth, cache folder; localized values, folder picker, descriptions under titles |
